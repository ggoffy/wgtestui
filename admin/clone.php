...
$templateMain = 'wgtestui_admin_clone.tpl';
...
switch ($op) {
    ...
    default:
        ...
        $form  = new \XoopsThemeForm(\sprintf(\_AM_WGTESTUI_CLONE_TITLE, $helper->getModule()->getVar('name', 'E')), 'clone', 'clone.php', 'post', true);
        $clone = new \XoopsFormText(\_AM_WGTESTUI_CLONE_NAME, 'clone', 20, 20, '');
        $clone->setDescription(\_AM_WGTESTUI_CLONE_NAME_DSC);
    ...
    case 'submit':
        ...
        if (empty($clone) || \preg_match('/[^a-zA-Z0-9\_\-]/', $clone)) {
            \redirect_header('clone.php', 3, \sprintf(\_AM_WGTESTUI_CLONE_INVALIDNAME, $clone));
        }

        // Check wether the cloned module exists or not
        $dirClone = $GLOBALS['xoops']->path('modules/' . $clone);
        if ($clone && \is_dir($dirClone)) {
            \redirect_header('clone.php', 3, \sprintf(\_AM_WGTESTUI_CLONE_EXISTS, $clone));
        }

        $patterns = [
            \mb_strtolower(\WGTESTUI_DIRNAME)           => \mb_strtolower($clone),
            \mb_strtoupper(\WGTESTUI_DIRNAME)           => \mb_strtoupper($clone),
            \ucfirst(\mb_strtolower(\WGTESTUI_DIRNAME)) => \ucfirst(\mb_strtolower($clone)),
        ];

        $patKeys   = \array_keys($patterns);
        $patValues = \array_values($patterns);
        cloneFileFolder(\WGTESTUI_PATH);
        ...
        $content = \str_replace('Wgtestui', \mb_strtolower($clone), $content);
        file_put_contents($dirClone . '/language/english/modinfo.php', $content);

        $msg = '';
        if (\is_dir($GLOBALS['xoops']->path('modules/' . \mb_strtolower($clone)))) {
            $msg .= \sprintf(\_AM_WGTESTUI_CLONE_CONGRAT, "<a href='" . \XOOPS_URL . "/modules/system/admin.php?fct=modulesadmin&op=installlist'>" . \ucfirst(\mb_strtolower($clone)) . '</a>') . "<br>\n";
            if (!$logocreated) {
                $msg .= \_AM_WGTESTUI_CLONE_IMAGEFAIL;
            }
        } else {
            $msg .= \_AM_WGTESTUI_CLONE_FAIL;
        }
